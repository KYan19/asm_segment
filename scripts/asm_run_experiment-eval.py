...
run_name = "12_all_data_lowlr_w/eval"

datamodule = ASMDataModule(batch_size=batch_size, num_workers=num_workers, split=split, split_n=split_n, root=root, transforms=min_max_transform, mines_only=mines_only, split_path=split_path)
...
checkpoint_callback = ModelCheckpoint(every_n_epochs=n_epoch-1) # save only final epoch model
